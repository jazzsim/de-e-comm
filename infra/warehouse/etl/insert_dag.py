...
def insert_customer_dim():
    ...
    sql = text(f"""
               ...
               FROM {source_table} t WHERE t.created_at > '{yesterday_date()}'
    """) 

    upsert(source_table, DimCustomer, sql, CUSTOMERS_KEYS, insert=True)

def insert_address_dim():
    ...
    sql = text(f"""
               SELECT * FROM {source_table} t WHERE t.created_at > '{yesterday_date()}'
    """) 

    upsert(source_table, DimAddress, sql, ADDRESSES_KEYS, insert=True)

def insert_category_dim():
    ...
    upsert(source_table, DimCategory, sql, CATEGORIES_KEYS, insert=True)
...
def insert_product_dim():
    ...
    upsert(source_table, DimProduct, sql, PRODUCTS_KEYS, insert=True)
