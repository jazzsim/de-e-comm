...
def initialize_warehouse_db():
    ...
    def purge():
        ...
        try:
            pg_hook.run("TRUNCATE TABLE dim_customer, dim_address, dim_category, dim_product, dim_customer_support, fact_sales, fact_payments, fact_order_details, fact_cart CASCADE", autocommit=True)
    ...
    def initDimDate():
        ...
        with open('/opt/airflow/dags/sql/dim_date.sql', 'r') as f:
            sql = f.read()
        pg_hook.run(sql, autocommit=True)
